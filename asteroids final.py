...
def group_collide(group, obj):
    global lives, score, missile_group, rock_group
    for x in group:
        if x.collide(obj) == True:
            if obj == my_ship:
                lives = lives - 1
                rock_group.pop(rock_group.index(x))
            else:
                score = score + 1
                missile_group.pop(missile_group.index(obj))
                rock_group.pop(rock_group.index(x))
...
def draw(canvas):
    ...
    for x in missile_group:
        ...
        x.age = x.age+1
        if x.age == x.lifespan:
            missile_group.pop(missile_group.index(x))
    ...
    if lives == 0:
        ...
        for x in rock_group:
            rock_group.pop(rock_group.index(x))
        for x in missile_group:
            missile_group.pop(missile_group(missile_group.index(x)))
...
def rock_spawner():
    ...
    if dist(rock_pos, my_ship.get_pos()) <= my_ship.get_radius():
        pass
    else:
